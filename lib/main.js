...
settings.on('disable', function (prefName, newValue) {
    postEvent(newValue ? 'AutoPagerizeDisableRequest' : 'AutoPagerizeEnableRequest')
})
...
exports.main = function (options, callbacks) {
    ...
    contextMenu.Menu({
        ...
        items: [
            (function () {
                var toggle = contextMenu.Item({
                    label: settings.get("disable") ? "on" : "off",
                    data: "toggle"
                })
                settings.on("disable", function (key, newValue) {
                    toggle.label = settings.get("disable") ? "on" : "off"
                })
                return toggle
            })(),
        ...
        onMessage: function (message) {
            if (message == 'show_config_panel') {
                ...
                settings.set('disable', !settings.get('disable'))
            }
        }
    })
    ...
    var configPanel = panels.Panel({
        ...
        onMessage: function(message) {
            if (message.name == 'settings') {
                configPanel.postMessage({ name: message.name, data: settings.getAll() })
            }
            else if (message.name == 'settingsUpdate') {
                for (var key in message.data) {
                    settings.set(key, message.data[key])
                }
                configPanel.hide()
